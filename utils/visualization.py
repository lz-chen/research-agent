...
def visualize_nodes_with_attributes(nodes: List[Union[IndexNode, TextNode]], graph_name_prefix: str = ""):
    G = nx.Graph()
        ...
        for r_type, r_info in node.relationships.items():
            if r_type.name == "SOURCE":
                ...
                if src_node_id:  # can found the src node in the node list
                    if src_node_id != node.id_:  # only add relation when it's not referring to itself
    ...
    net = Network(notebook=False, cdn_resources='in_line', width="100%", height="1500px", bgcolor="#ffffff",
                  font_color="black")
    net.from_nx(G)
